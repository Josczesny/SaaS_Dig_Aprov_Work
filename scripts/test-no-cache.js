...
async function testNoCache() {
  ...
  try {
    ...
    // 2. Testar exportação com headers que forçam sem cache
    console.log('\n2. Testando exportação sem cache...');
    
    const csvResponse = await axios.get('http://localhost:3000/api/audit/export/csv', {
      params: {
        startDate: '2025-07-03',
        endDate: '2025-08-03',
        _t: Date.now() // Forçar sem cache
      },
      headers: {
    ...
    console.log('✅ CSV - Headers:', csvResponse.headers);
    ...
    const simpleResponse = await axios.get('http://localhost:3000/', {
      headers: {
